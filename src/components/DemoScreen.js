...
import { Dimensions, Text, SafeAreaView, View, Alert } from "react-native";
...
export default class Home extends React.Component {
  static navigationOptions = ({ navigation }) => {
    return {
      title: "DEMO",
      headerStyle: { height: 70, backgroundColor: "rgb(246,205,101)" },
      ...
      headerRight: (
        <View style={{ paddingRight: 15 }}>
          ...
          >
            <Text style={{ fontSize: 20, color: "red" }}>BES+</Text>
  ...
  componentDidMount() {
    this.props.navigation.addListener("willFocus", payload => {
      this.dropDownAlertRef.alertWithType(
        "info",
        "Bu ekranda değişiklikler yapabilirsiniz!",
        "Burada yaptığınız işlemler gerçek BES fonunuzu etkilemeyecektir."
      );
    });
